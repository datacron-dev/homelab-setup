...
show_main_menu() {
    while true; do
        CHOICES=$(dialog --menu "Select programs to install" 20 78 10 \
...
show_model_menu() {
    while true; do
        MODELS=$(dialog --menu "Select models to download via Ollama" 20 78 10 \
            "Llama 3.1 8B (~4.7 GB)" ON \
            "Llama 3.1 70B (~40 GB)" OFF)
        RET=$?
        if [[ $RET -eq 0 ]]; then
            local cleaned
